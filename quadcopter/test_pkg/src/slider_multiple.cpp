...
{
    if (posePtr -> header.frame_id.compare("Gypsy Danger")==0){
	k=0;
	}
else if (posePtr -> header.frame_id.compare("Typhoon")==0){
	k=1;
	}
else if (posePtr -> header.frame_id.compare("dummy 1")==0){
	k=101;
	}
else if (posePtr -> header.frame_id.compare("dummy 2")==0){
	k=102;
	}
	

    poseEstimation[k].pose = posePtr -> pose;
    poseSysId[k].x = poseEstimation[k].pose.position.x; // Update current pose estimation data
    poseSysId[k].y = poseEstimation[k].pose.position.y;
    poseSysId[k].z = poseEstimation[k].pose.position.z;
    poseEstYaw[k] = tf::getYaw(poseEstimation[k].pose.orientation) + PI;
    active[k]=true;
}

void velocityEstCallback(const geometry_msgs::TwistStamped::ConstPtr& twistPtr)
{
if (twistPtr -> header.frame_id.compare("Gypsy Danger")==0){
	k=0;
	}
else if (twistPtr -> header.frame_id.compare("Typhoon")==0){
	k=1;
	}
else if (twistPtr -> header.frame_id.compare("dummy 1")==0){
	k=101;
	}
else if (twistPtr -> header.frame_id.compare("dummy 2")==0){
	k=102;
	}
    velEstimation[k].linear = twistPtr->twist.linear;
}
...
void PID(int i)
{
    ...
    velPoseEstX.x = poseEstimation[i].pose.position.x;
    velPoseEstX.y = velocity.transforms[i].transform.translation.x;
}
...
int main(int argc, char **argv)
{
    ...
    velEstSub = n.subscribe<geometry_msgs::TwistStamped>("/velocityEstimation", 1, velocityEstCallback);
