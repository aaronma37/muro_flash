...
//    pastZ = poseSysId[i].z;
    int i=0;
    poseEstimation[i].pose = posePtr -> pose;
    poseSysId[i].x = poseEstimation[i].pose.position.x; // Update current pose estimation data
...
{
    poseErrorPrev.pose[i] = poseError[i].pose; // Store old pose error for PID equation
...
void PID(int i)
{
    ...
    velocity[i].linear.x = sX[i]*(-sliderGain) + (kd*T*(maResults[0])) + pathVel.linear.x;
    if (velocity[i].linear.x > 1){
      velocity[i].linear.x = 1;
    }
    else if (velocity[i].linear.x < -1){
      velocity[i].linear.x = -1;
    }
    
    velocity[i].linear.y = sY[i]*sliderGain + (kd*T*(maResults[1])) + pathVel.linear.y;
    if (velocity[i].linear.y > 1){
      velocity[i].linear.y = 1;
    }
    else if (velocity[i].linear.y < -1){
      velocity[i].linear.y = -1;
    }
    
    velocity[i].linear.z = sZ[i]*-(sliderGain);
    if (velocity[i].linear.z > 1){
      velocity[i].linear.z = 1;
    }
    else if (velocity[i].linear.z < -1){
      velocity[i].linear.z = -1;
    }
    
    velocity[i].angular.z = (kpYaw*poseErrYaw[i]) + (kiYaw*pastYawErr[i]) + (kdYaw*T*(maResults[3]));
    
      if (velocity[i].linear.x!=0 && velocity[i].linear.y!=0){
		    double dummyA=1;
		    double dummyO=tan(poseEstYaw[i]+PI);
		    double norm1=sqrt((velocity[i].linear.x*velocity[i].linear.x+velocity[i].linear.y*velocity[i].linear.y));


		    double dot = dummyA*velocity[i].linear.x + dummyO*velocity[i].linear.y;   
		    double det = dummyA*velocity[i].linear.y - dummyO*velocity[i].linear.x;   
		    double activeAngle = atan2(det, dot);

		    velocity[i].linear.x =  norm1*cos(activeAngle);
		    velocity[i].linear.y =  norm1*sin(activeAngle);
   	 }
    
    // For modeling purposes
    velPoseEstX.x = poseEstimation.pose.position.x;
    velPoseEstX.y = velocity[i].linear.x;
}
